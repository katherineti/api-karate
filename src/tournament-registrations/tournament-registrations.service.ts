      ...
      .where(
        and(
          ...
          // 3. Que tengan el rol de alumno (ID 5)
           // Usamos @> porque roles_ids es un jsonb array en el schema
...
async getSchoolsByDivision(modalityId: number) {
  ...
  /**
   * MÉTODO 0: Obtener eventos con estado de inscripción (Alumno)
   * (Alumno) Gestión de Eventos
   */
  async getEventsWithEnrollmentStatus(athleteId: number, query: PaginationTournamentRegistrationsDto) {
    ...
    try {
      const offset = (page - 1) * limit;
        ...
        .leftJoin(
          tournamentRegistrationsTable,
           and(
             eq(tournamentRegistrationsTable.event_id, eventsTable.id),
        ...
        // .leftJoin(eventCategoriesTable, eq(eventsTable.id, eventCategoriesTable.event_id))
        // .leftJoin(eventDivisionsTable, eq(eventCategoriesTable.id, eventDivisionsTable.event_category_id))
        .leftJoin(
          tournamentRegistrationsTable,
           and(
  ...
  /**
   * MÉTODO 1 - Solicitud sencilla: Crear solicitud de participación/pre-inscripcion a un torneo (Alumno).
   * (Alumno) Gestión de Eventos > Accion 'Solicitar Participación'
   * 
   * Flujo:
  ...
  async createParticipationRequest(athleteId: number, eventId: number) {
    try {
      ...
      if (existingReg) {
        throw new BadRequestException('Ya tienes una solicitud de participación/pre-inscripción para este torneo.');
      }

      // 3. Crear la solicitud (sin categoría ni modalidad, las asigna el Master después)
      ...
      if(newRegistration){
          const [alumno] = await this.db.select({name:usersTable.name, lastname: usersTable.lastname }).from(usersTable).where(eq(usersTable.id, athleteId))
      ...
      throw new InternalServerErrorException('Error al crear la solicitud de participación/pre-inscripción al torneo.');
    }
  }
