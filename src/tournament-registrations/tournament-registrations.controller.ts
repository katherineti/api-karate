import { Controller, Get, Post, Body, Patch, Param, Delete, ValidationPipe, UsePipes, ParseIntPipe, Query } from '@nestjs/common';
...
@Post('bulk')
  ...
  async bulkRegister(
    ...
    return this.tournamentRegistrationsService.bulkRegisterAthletes({
      event_id: dto.event_id,      // ID del evento
      category_id: dto.category_id,   // ID de la categoría
      modality_id: dto.modality_id,   // ID de la modalidad
...
//  @Roles(2) // ID del rol master en tu sistema
  ...
  @Roles(5) // ID del rol alumno
  async getMyEvents(
    @Query() query: PaginationTournamentRegistrationsDto,
    @Usersesion() user: IJwtPayload,
  ) {
